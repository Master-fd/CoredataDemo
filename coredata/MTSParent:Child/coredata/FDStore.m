...
- (NSManagedObjectModel *)manageObjectModel
{
    if (!__managedObjectModel) {
        NSURL *url = [[NSBundle mainBundle] URLForResource:@"parentChildDepartment" withExtension:@"momd"];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    if (!__persistentStoreCoordinator) {
        ...
        //连接对应的数据库
        NSURL *url = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"parentData.sqlite"];
        FDLog(@"数据库地址：%@", url);
        NSError *error=nil;
